...
import shs2.rohfeedback.database.binder.ParameterBinder;
import shs2.rohfeedback.database.mapper.RowMapper;
import shs2.rohfeedback.utility.SmartLog;
import shs2.rohfeedback.utility.StringUtility;

//import com.shss.restaurantwaiter.database.binder.ParameterBinder;
//import com.shss.restaurantwaiter.database.mapper.RowMapper;
...
public class PrepareStatement {
	...
	public boolean insert(String sql, Object object,
		...
		if (db != null) {
			...
			try {
				...
				statement.execute();
				success = true;
				SmartLog.log(TAG, MESSAGE_INSERT_AN_OBJECT_SUCCESSFULLY
						+ MESSAGE_LINE);
			} catch (Exception ex) {
				SmartLog.log(TAG, MESSAGE_ERROR_WHEN_IMPLEMENT_QUERY + " : "
						+ sql);
	...
	private SQLiteDatabase openDatabaseConnection() {
		// Open database connection
		return (new OpenDBHelper(context, DatabaseConfig.getInstance()))
				.getWritableDatabase();
	}
